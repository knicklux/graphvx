...
class ADMMNesterovResetIteration(IterationScheme):
    ...
    def prepare(
        ...
        self.z_hat = np.zeros((nz,), dtype=np.float64)
        self.u_hat = np.zeros((nz,), dtype=np.float64)
    ...
    def iteration(self, k: int, rho: float):
        ...
        # x-update
        self.iteration_solver_x.set_primal(self.z_hat)
        ...
        self.u = self.u_hat + rho * (Ax - self.z)

        self.c = ((1./rho) * np.inner(self.u-self.u_hat, self.u-self.u_hat) +
                  rho * np.inner(self.z-self.z_hat, self.z-self.z_hat))

        print(f"c: {self.c}, c_old: {self.c_old}")

        if self.c < self.eta * self.c_old:

            print("Accelerated step accepted")
            self.alpha = (1. + np.sqrt(1 + 4*self.alpha**2)) / 2.
            self.z_hat = self.z + (self.alpha - 1) / (self.alpha + 1) * (self.z - self.z_old)
            self.u_hat = self.u + (self.alpha - 1) / (self.alpha + 1) * (self.u - self.u_old)
            ...
            print("Reset")
            self.alpha = 1.
            self.z_hat = self.z_old
            self.u_hat = self.u_old
            self.c_old = self.c_old / self.eta

        self.iteration_solver_z.set_dual(self.u_hat)

    def check_convergence(self, rho, e_abs, e_rel):
        norm = np.linalg.norm
        Ax = self.A @ self.x
        r = Ax - self.z
        s = rho * self.A_tr.dot(self.z - self.z_hat)
